...
def phenotrex_parsing():
    phenotrex_dir = '//wsl.localhost/Ubuntu/home/vitt\\validation_sets\\sporulation_validation\\phenotrex_outputs'
    ...
    for root, subfolders, files in os.walk(phenotrex_dir):
        for file in files:
            ...
            if fnmatch.fnmatch(file, '*sporulation.svm.class.csv*'):
                #print("FOUND:", file)
                try:
    ...
    print(phenotrex_SGB)
    phenotrex_SGB.to_csv('//wsl.localhost/Ubuntu/home/vitt/phenotrex_sporulation_validation.npy')
...
def traitar_parsing():
    ...
    traiatar_dir = '//wsl.localhost/Ubuntu/home/vitt\\validation_sets\\sporulation_validation\\traitar_outputs'
    ...
    for subset_dir in os.listdir(traiatar_dir):
        ...
        for main_dir in os.listdir(subset_dir):
            ...
            temp_d = {"SGB": SGB, "Spore formation": "NO"}

            # PROCESSING
            for row in temp["sample"]:
                if row == "Spore formation":
                    temp_d["Spore formation"] = "YES"
                
            row_list.append(temp_d)

    traitar = pd.DataFrame(row_list)
    print(traitar)
    traitar.to_csv('//wsl.localhost/Ubuntu/home/vitt/traitar_sporulation_validation.npy')
